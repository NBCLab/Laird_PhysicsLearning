...
for tmpsub in $subs; do
  if [[ $tmpsub == sub-* ]]; then
      if [ ! -d $proj_dir/dset/derivatives/fmriprep-1.5.0/$tmpsub ]; then
          ...
          echo $tmpsub
          while [ $(squeue -u $USER | wc -l) -gt 20 ]; do
              sleep 5m
          done
          sbatch -J $tmpsub-func-proc -e $proj_dir/code/err/$tmpsub-func-proc -o $proj_dir/code/out/$tmpsub-func-proc -n $nprocs --qos pq_nbc -p centos7_16C_32G --account iacc_nbc --wrap="python3 $proj_dir/code/fmriprep.py -b $proj_dir/dset -w $wkdir/$tmpsub-func-proc --sub $tmpsub --n_procs $nprocs"
      fi
  fi
done
