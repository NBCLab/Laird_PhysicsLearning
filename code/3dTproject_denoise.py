...
def main(argv=None):
    ...
    if args.ses is None:
        if not op.isdir(op.join(args.work_dir, args.sub)):
            shutil.copytree(op.join(args.bids_dir, 'derivatives/fmriprep-1.5.0', args.sub, 'fmriprep', args.sub), op.join(args.work_dir, args.sub))
    else:
        if not op.isdir(op.join(args.work_dir, args.sub, args.ses)):
            shutil.copytree(op.join(args.bids_dir, 'derivatives/fmriprep-1.5.0', args.sub, 'fmriprep', args.sub, args.ses, 'func'), op.join(args.work_dir, args.sub, args.ses, 'func'))
